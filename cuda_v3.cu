//compile : nvcc -o cuda_v3 cuda_v3.cu --expt-relaxed-constexpr
//run : ./cuda_v3
/*
...
__global__ void kernel5(float *hitmap, int no_of_features, int no_of_neurons,float *d_class_list, int no_of_classes){
      ...
      if(tid< no_of_neurons){
        ...
        int max_idx=0;
            ...
            for(int col=0; col< no_of_classes; col++){
                if(hitmap[tid  * no_of_classes + col] >= max){
                    max = hitmap[tid  * no_of_classes + col];
                    max_idx = col;
                }
            }
            d_class_list[tid] = max_idx;
      }

}

__global__ void kernel6( float *d_traindate, float *d_traindata_class, float *d_weight_matrix, int no_of_features, int no_of_neurons,float *d_class_list,int no_of_classes, float *d_predicted_labels, int no_of_training_data){
      int tid = blockDim.x* blockIdx.x + threadIdx.x;
      if(tid< no_of_training_data){
        ...
        for(int neu=0; neu<no_of_neurons; neu++){
            ...
            for(int col=0; col< no_of_features; col++){
                distance += abs( d_traindate[tid* no_of_features + col] - d_weight_matrix[ neu * no_of_features + col]) ;
            }
            if(distance<=min_dis){
                min_dis = distance;
                min_idx = neu;
        ...
        d_predicted_labels[tid]=0;
        if(d_traindata_class[tid]== d_class_list[min_idx]){
            d_predicted_labels[tid]=1;
        }
        __syncthreads();
        if(tid == 0){
          float sum=0;
          for(int d=0; d<no_of_training_data; d++){
            sum+=d_predicted_labels[d];
          }
          printf("\n Tarin acc in Kernel %f ", sum/no_of_training_data);
...
int main(int argc, char *argv[]){
    ...
    float *d_class_list, *d_hit_map, *d_traindata_class, *d_predicted_labels;
    int *d_bmu;
    ...
    cudaMalloc(&d_predicted_labels, no_of_training_data*1* sizeof(float));
    ...
    cudaMemcpy(d_predicted_labels, traindata_class, no_of_training_data*1 * sizeof(float), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(d_hit_map, hitmap, no_of_neurons* no_of_classes * sizeof(float), cudaMemcpyHostToDevice);
    clock_t end = clock();
    double elapsed_secs_training_p = double(end - begin) / CLOCKS_PER_SEC;
    ...
    for(int row = 0; row < no_of_training_data; row++)//no_of_training_data
    {
              ...
              hitmap[min_idx  * no_of_classes + int(traindata_class[row])] = hitmap[min_idx  * no_of_classes + int(traindata_class[row])] +1;
    }
    end = clock();
    double elapsed_secs_calculating_hitmap_s = double(end - begin) / CLOCKS_PER_SEC;
    ...
    cudaMemcpy(hitmap, d_hit_map, no_of_neurons* no_of_classes * sizeof(float), cudaMemcpyDeviceToHost);
    end = clock();
  	double elapsed_secs_calculating_hitmap_p = double(end - begin) / CLOCKS_PER_SEC;

    cout << endl;
    //print_hits(som_dim*som_dim, no_of_classes, hitmap);
    begin = clock();
    cudaEventRecord(start);
    kernel5<<<grid_dim_k1, threads_per_block_k1>>>(d_hit_map, no_of_features, no_of_neurons,d_class_list,no_of_classes);
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);

    cudaMemcpy(class_list, d_class_list, no_of_neurons * sizeof(float), cudaMemcpyDeviceToHost);
    end = clock();
    double elapsed_secs_calculating_class_assignment_p = double(end - begin) / CLOCKS_PER_SEC;
    ...
    for(int neu=0; neu<no_of_neurons; neu++){
  	...
  	double elapsed_secs_calculating_class_assignment_s = double(end - begin) / CLOCKS_PER_SEC;
    ...
    for(int row = 0; row < no_of_training_data; row++)//no_of_training_data
    {
  	...
  	double elapsed_secs_testing_accuracy_calc_s = double(end - begin) / CLOCKS_PER_SEC;


    begin = clock();
    int threads_per_block_k5 = 1024;
    int grid_dim_k5 = (no_of_training_data + threads_per_block_k5 - 1) / threads_per_block_k5;
    cudaMemcpy(d_traindate, traindata, no_of_training_data* no_of_features * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_traindata_class, traindata_class, no_of_training_data*1 * sizeof(float), cudaMemcpyHostToDevice);

    cudaEventRecord(start);
    kernel6<<<grid_dim_k5, threads_per_block_k5>>>(d_traindate,d_traindata_class,d_weight_matrix, no_of_features, no_of_neurons,d_class_list,no_of_classes, d_predicted_labels,no_of_training_data);
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    end = clock();
  	double elapsed_secs_testing_accuracy_calc_p = double(end - begin) / CLOCKS_PER_SEC;


    cout << endl;

    printf("No of testing data %d: train acc: %f \n", no_of_training_data, correct_count/float(no_of_training_data));
    cout << "\nTraining time in Seconds (claculation weight matrix and fill the hitmap) : " << elapsed_secs_training_p <<endl;
    cout << "\nHitmap Calculation time in Seconds Serial : " << elapsed_secs_calculating_hitmap_s <<endl;
    cout << "\nHitmap Calculation time in Seconds Para : " << elapsed_secs_calculating_hitmap_p <<endl;
    cout << "\nClass assignment calculation time in Seconds (find majority voted class and trin accuracy using hitmap) serial : " << elapsed_secs_calculating_class_assignment_s <<endl;
    cout << "\nClass assignment calculation time in Seconds (find majority voted class using hitmap) parallel : " << elapsed_secs_calculating_class_assignment_p <<endl;
    cout << "\nTest accuracy calculation time in Seconds CPU serial : " << elapsed_secs_testing_accuracy_calc_s <<endl;
    cout << "\nTest accuracy calculation time in Seconds GPU: parallel" << elapsed_secs_testing_accuracy_calc_p <<endl;

    cout << "\nTotal Parallel time GPU: " << elapsed_secs_training_p+ elapsed_secs_calculating_hitmap_p+ elapsed_secs_calculating_class_assignment_p + elapsed_secs_testing_accuracy_calc_p <<endl;

    double total_p =elapsed_secs_training_p+ elapsed_secs_calculating_hitmap_p+ elapsed_secs_calculating_class_assignment_p + elapsed_secs_testing_accuracy_calc_p;
    double temp_s =  elapsed_secs_calculating_hitmap_s+ elapsed_secs_calculating_class_assignment_s + elapsed_secs_testing_accuracy_calc_s;

    string fname= to_string(no_of_neurons)+"_"+to_string(no_of_training_data)+".csv";
    ofstream myFile(fname);
    myFile << elapsed_secs_training_p;
    myFile << "\n";
    myFile << elapsed_secs_calculating_hitmap_s;
    myFile << "\n";
    myFile << elapsed_secs_calculating_hitmap_p;
    myFile << "\n";
    myFile << elapsed_secs_calculating_class_assignment_s;
    myFile << "\n";
    myFile << elapsed_secs_calculating_class_assignment_p;
    myFile << "\n";
    myFile << elapsed_secs_testing_accuracy_calc_s;
    myFile << "\n";
    myFile << elapsed_secs_testing_accuracy_calc_p;
    myFile << "\n";
    myFile << total_p;
    myFile << "\n";
    myFile << temp_s;
